...
class ExcelWriter:
    ...
    def write_経費助成内訳_定額制(self, company: CompanyInfo, group: CurriculumGroup) -> str:
        ...
        fname = f"04_様式6-3号(定額制)_未生成_{group.curriculum_name}_{company.company_name}.xlsx"
        return self._patch(
    ...
    def write_賃金助成内訳(self, company: CompanyInfo, group: CurriculumGroup) -> str:
        ...
        修正点（PPT指摘事項）:
          ...
          - P7 は明示的に空欄でクリア (テンプレートに残った値や旧データの上書き対策)
        ...
        values = {
            ...
            'P7': '',
            # 雇用保険適用事業所の名称 (offices[0].name を優先、空なら company_name)
